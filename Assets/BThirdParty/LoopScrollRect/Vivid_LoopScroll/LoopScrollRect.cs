...
namespace VIVID
{
	...
	{
		public delegate void OnUpdateDrag();
		public event OnUpdateDrag onUpdateDrag;

		public Action onAutoScrollEnd;
		//==========LoopScrollRect==========
		...
		//	{
		//		//if (value != null)
		//		//	dataSource = new SCLoopScrollArraySource<object>(value);
		//		//else
		...
		protected int m_item_type_start = 0;
		protected int m_item_type_end = 0;
		...
		private bool m_content_space_init = false;
		private float m_content_spacing = 0;
		protected GridLayoutGroup m_grid_layout = null;
		protected float contentSpacing
		{
			get
			{

				if (m_content_space_init)
				{
					return m_content_spacing;
				}
				m_content_space_init = true;
				m_content_spacing = 0;
				if (content != null)
				{
					...
					if (layout1 != null)
					{
						m_content_spacing = layout1.spacing;
					}
					m_grid_layout = content.GetComponent<GridLayoutGroup>();
					if (m_grid_layout != null)
					{
						m_content_spacing = Mathf.Abs(GetDimension(m_grid_layout.spacing));
					}
				}
				return m_content_spacing;
			}
		...
		private bool m_content_constraint_count_init = false;
		private int m_content_constraint_count = 0;
		protected int contentConstraintCount
		{
			get
			{
				if (m_content_constraint_count_init)
				{
					return m_content_constraint_count;
				}
				m_content_constraint_count_init = true;
				m_content_constraint_count = 1;
				if (content != null)
				{
					...
					if (layout2 != null)
					{
						...
						m_content_constraint_count = layout2.constraintCount;
					}
				}
				return m_content_constraint_count;
			}
		...
		public int StartLine
		{
			get
			{
				return Mathf.CeilToInt((float)(m_item_type_start) / contentConstraintCount);
			}
		}
		...
		public int CurrentLines
		{
			get
			{
				return Mathf.CeilToInt((float)(m_item_type_end - m_item_type_start) / contentConstraintCount);
			}
		}
		...
		[SerializeField] [Tooltip("실행 중에 변경 하지마세요")] private bool useScrollRectForBounds;
		[SerializeField]
		...
		//==========LoopScrollRect==========
		public T FindItem<T>(Int32 in_index)
		{
			T target = default;
			for (int i = 0 ; i < content.childCount ; i++)
			{
				T component = content.GetChild(i).GetComponent<T>();
				...
				VIVID.ILoopItem loop_item = component as VIVID.ILoopItem;
				if (loop_item == null)
				{
					continue;
				}

				if (loop_item.Index == in_index)
				{
					target = content.GetChild(i).GetComponent<T>();
					break;
				}
			}
		...
		void ScrollToCellInstant(int index, float speed)
		{
			if (!m_dragging)
			{
				SetViewBounds(viewRect.rect, scrollRect.rect);
				...
				if (totalCount >= 0)
				{
					if (offset > 0 && m_item_type_end == totalCount && !reverseDirection)
					{
					...
					else if (offset < 0 && m_item_type_start == 0 && reverseDirection)
					{
		...
		IEnumerator ScrollToCellCoroutine(int index, float speed)
		{
			...
			while (needMoving)
			{
				...
				{
					float move = 0;
					if (index < m_item_type_start)
					{
						move = -Time.deltaTime * speed;
					}
					else if (index >= m_item_type_end)
					{
						move = Time.deltaTime * speed;
					}
					else
					{
						...
						var m_item_bounds = GetBounds4Item(index);
						var offset = 0.0f;
						if (directionSign == -1)
						{
							offset = reverseDirection ? (m_ViewBounds.min.y - m_item_bounds.min.y) : (m_ViewBounds.max.y - m_item_bounds.max.y);
						}
						else if (directionSign == 1)
						{
							offset = reverseDirection ? (m_item_bounds.max.x - m_ViewBounds.max.x) : (m_item_bounds.min.x - m_ViewBounds.min.x);
						}
						// check if we cannot move on
						if (totalCount >= 0)
						{
							if (offset > 0 && m_item_type_end == totalCount && !reverseDirection)
							{
								m_item_bounds = GetBounds4Item(totalCount - 1);
								// reach bottom
								if ((directionSign == -1 && m_item_bounds.min.y > m_ViewBounds.min.y) ||
									(directionSign == 1 && m_item_bounds.max.x < m_ViewBounds.max.x))
								{
									needMoving = false;
									break;
								}
							...
							{
								m_item_bounds = GetBounds4Item(0);
								if ((directionSign == -1 && m_item_bounds.max.y < m_ViewBounds.max.y) ||
									(directionSign == 1 && m_item_bounds.min.x > m_ViewBounds.min.x))
								{
									needMoving = false;
									break;
								}
		...
		public void RefreshCells()
		{
			if (Application.isPlaying && this.isActiveAndEnabled)
			{
				...
				for (int i = 0 ; i < content.childCount ; i++)
				{
					if (m_item_type_end < totalCount)
					{
						dataSource.ProvideData(content.GetChild(i), m_item_type_end);
						m_item_type_end++;
					}
					else
					{
		...
		public bool IsEndList(Int32 in_index)
		{
			RectTransform prefab = prefabSource.m_prefab_source.transform as RectTransform;

			Int32 count = (Int32)(viewRect.rect.size.y / prefab.sizeDelta.y);

			Int32 limit = reverseDirection ? count : totalCount - count - 1;
			if (reverseDirection)
			{
				return in_index < limit;
			}
			else
			{
				return in_index > limit;
		...
		public void RefillCellsFromEnd(int in_offset = 0, bool in_align_start = false, bool in_is_init = false)
		{
			...
			StopMovement();
			m_item_type_end = reverseDirection ? in_offset : totalCount - in_offset;
			m_item_type_start = m_item_type_end;

			if (totalCount >= 0 && m_item_type_start % contentConstraintCount != 0)
			{
				m_item_type_start = (m_item_type_start / contentConstraintCount) * contentConstraintCount;
			}
			...
			{
				float size = reverseDirection ? NewItemAtEnd(in_is_init) : NewItemAtStart(in_is_init);
				if (size <= 0)
					break;
				sizeFilled += size;
			}
			...
			while (sizeToFill > sizeFilled)
			{
				float size = reverseDirection ? NewItemAtStart(in_is_init) : NewItemAtEnd(in_is_init);
				if (size <= 0)
					break;
				sizeFilled += size;
			}
			...
			float dist = in_align_start ? 0 : Mathf.Max(0, sizeFilled - sizeToFill);
			if (reverseDirection)
				dist = -dist;
			if (directionSign == -1)
				pos.y = dist;
		...
		public void RefillCells(int offset = 0, bool fillViewRect = false, bool in_is_init = false)
		{
			if (!Application.isPlaying || prefabSource == null)
				return;

			StopMovement();
			m_item_type_start = reverseDirection ? totalCount - offset : offset;
			m_item_type_end = m_item_type_start;

			if (totalCount >= 0 && m_item_type_start % contentConstraintCount != 0)
			{
				m_item_type_start = (m_item_type_start / contentConstraintCount) * m_item_type_start;
			}
			...
			if (fillViewRect && itemSize > 0 && sizeFilled < sizeToFill)
			{
				...
				if (newOffset < 0)
					newOffset = 0;
				if (newOffset != offset)
					RefillCells(newOffset, in_is_init: in_is_init);                 //refill again, with the new offset value, and now with fillViewRect disabled.
			}
		...
		protected float NewItemAtStart(bool in_is_init = false)
		{
			if (totalCount >= 0 && m_item_type_start - contentConstraintCount < 0)
			{
				return 0;
			...
			{
				m_item_type_start--;
				RectTransform newItem = GetFromTempPool(m_item_type_start, in_is_init);
		...
		protected float DeleteItemAtStart()
		{
			...
			if ((m_dragging || m_Velocity != Vector2.zero) && totalCount >= 0 && m_item_type_end >= totalCount - contentConstraintCount)
			{
				return 0;
			}
		...
		protected float NewItemAtEnd(bool in_is_init = false)
		{
			if (totalCount >= 0 && m_item_type_end >= totalCount)
			{
				return 0;
			}
			float size = 0;
			...
			{
				RectTransform newItem = GetFromTempPool(m_item_type_end, in_is_init);
		...
		{
			if ((m_dragging || m_Velocity != Vector2.zero) && totalCount >= 0 && m_item_type_start < contentConstraintCount)
			{
				return 0;
			}
			int availableChilds = content.childCount - deletedItemTypeStart - deletedItemTypeEnd;
			...
			for (int i = 0 ; i < contentConstraintCount ; i++)
			{
				...
				availableChilds--;
				m_item_type_end--;
				if (m_item_type_end % contentConstraintCount == 0 || availableChilds == 0)
				{
		...
		protected RectTransform GetFromTempPool(int itemIdx, bool in_is_init = false)
		{
			...
			if (deletedItemTypeStart > 0)
			{
				...
				nextItem.SetSiblingIndex(itemIdx - m_item_type_start + deletedItemTypeStart);
			}
			...
			//if (in_is_init)
			//{
			//	dataSource.InitializeData(nextItem, itemIdx);
			//}
			//else
			//{
			//	dataSource.ProvideData(nextItem, itemIdx);
			//}
			bool set_provide = false;
			try
			{
				if (in_is_init)
				{
			...
			catch (Exception e)
			{
				if (set_provide)
				{
					PopAlert.CreateException(e, "DataSource.ProvideData 처리오류");
				}
				else
				{
					PopAlert.CreateException(e, "DataSource.InitialData 처리오류");
				}
			}
		...
		public float horizontalNormalizedPosition
		{
			get
			{
				UpdateBounds();
				//==========LoopScrollRect==========
				if (totalCount > 0 && m_item_type_end > m_item_type_start)
				{
					float elementSize = (m_ContentBounds.size.x - contentSpacing * (CurrentLines - 1)) / CurrentLines;
					float totalSize = elementSize * TotalLines + contentSpacing * (TotalLines - 1);
		...
		private void SetViewBounds(Rect in_view_rect, Rect in_scroll_rect)
		{

			if (in_scroll_rect == null)
			{
				m_ScrollBounds = new Bounds(in_view_rect.center, in_view_rect.size);
			}
			else
			{
				m_ScrollBounds = new Bounds(in_scroll_rect.center, in_scroll_rect.size);
			}

			m_ViewBounds = new Bounds(in_view_rect.center, in_view_rect.size);
		...
		private void UpdateBounds(bool updateItems = false)
		{
			...
			Bounds bounds = m_ViewBounds;

			if (useScrollRectForBounds)
			{
				bounds = m_ScrollBounds;
			}

			excess = bounds.size - contentSize;
			if (excess.x > 0)
			{
				contentPos.x -= excess.x * (m_Content.pivot.x - 0.5f);
				contentSize.x = bounds.size.x;
			}
			if (excess.y > 0)
			{
				contentPos.y -= excess.y * (m_Content.pivot.y - 0.5f);
				contentSize.y = bounds.size.y;
			}
		...
		private Bounds GetScrollBounds()
		{
			...
			var toLocal = viewRect.worldToLocalMatrix;
			m_scrollRect.GetWorldCorners(m_Corners);
			for (int j = 0 ; j < 4 ; j++)
			{
				Vector3 v = toLocal.MultiplyPoint3x4(m_Corners[j]);
				vMin = Vector3.Min(v, vMin);
		...
		private Bounds GetBounds4Item(int index)
		{
			...
			var toLocal = viewRect.worldToLocalMatrix;
			int offset = index - m_item_type_start;
		...
		private Vector2 CalculateOffset(Vector2 delta)
		{
			...
			if (m_MovementType == MovementType.Clamped)
			{
				if (totalCount < 0)
					return offset;
				if (GetDimension(delta) < 0 && m_item_type_start > 0)
					return offset;
				if (GetDimension(delta) > 0 && m_item_type_end < totalCount)
					return offset;
			}
