...
namespace Min_Max_Slider
{
	...
	public class MinMaxSlider : Selectable, IBeginDragHandler, IDragHandler, IEndDragHandler
	{
		...
		protected override void Start()
		{
			...
			else if (parentCanvas.worldCamera != null)
			{
		...
		// 핸들이 끝에 닿았을 때 반대쪽 핸들이 끌려오도록 하고싶다면 true로 셋 해줘야함
		public void SetIsBoth(bool isUse)
		{
			isUseBoth = isUse;
		}
		...
		public void OnBeginDrag(PointerEventData eventData)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						float min_distance = Math.Abs(dragStartMinValue01 - dragStartValue);
						float max_distance = Math.Abs(dragStartMaxValue01 - dragStartValue);
