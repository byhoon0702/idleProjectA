...
public class DailyRewardsEventManager : MonoBehaviour
{
	...
	public DailyReward GetDailyRewards(int dayIndex) => m_Status.dailyRewards[dayIndex];

	//public List<DailyReward> bonusReward => m_Status.bonusReward;

	public DailyReward rewardsGranted { get; private set; }
	...
	public async Task RefreshDailyRewardsEventStatus()
	{
		try
		{
			m_Status = await DailyRewards.CloudCodeManager.Instance.CallGetStatusEndpoint();
			if (this == null) return;
	...
	public async Task Demonstration_StartNextMonth()
	{
		try
		{
			...
			await DailyRewards.CloudCodeManager.Instance.CallResetEventEndpoint();
			if (this == null) return;
	...
	public async Task ClaimDailyReward()
	{
		try
		{
			...
			var claimResult = await DailyRewards.CloudCodeManager.Instance.CallClaimEndpoint();
			if (this == null) return;
			...
			if (claimResult.rewardsGranted.tid != 0)
			{
				var reward = claimResult.rewardsGranted;
				AddItemInfo info = new AddItemInfo(reward.tid, (IdleNumber)reward.quantity, RewardCategory.Currency);
				PlatformManager.UserDB.AddRewards(new List<AddItemInfo>() { info }, true, true);
			}


			Debug.Log("CloudCode script rewarded: " + $"{string.Join(",", claimResult.rewardsGranted)}.");
	...
	public struct ClaimResult
	{
		...
		public List<DailyReward> dailyRewards;
		//public List<DailyReward> bonusReward;
		public DailyReward rewardsGranted;
		...
		public override string ToString()
		{
			...
			return outputString + $" rewardsGranted:{string.Join(",", rewardsGranted)}";
		}
	}
