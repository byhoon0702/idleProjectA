...
namespace DailyRewards
{
	public class CloudCodeManager : MonoBehaviour
	{
		...
		public async Task<DailyRewardsEventManager.GetStatusResult> CallGetStatusEndpoint()
		{
			...
			try
			{
				return await CloudCodeService.Instance.CallEndpointAsync<DailyRewardsEventManager.GetStatusResult>("DailyRewards_GetStatus", new Dictionary<string, object>());
		...
		public async Task<DailyRewardsEventManager.ClaimResult> CallClaimEndpoint()
		{
			...
			try
			{
				return await CloudCodeService.Instance.CallEndpointAsync<DailyRewardsEventManager.ClaimResult>("DailyRewards_Claim", new Dictionary<string, object>());
			}
			catch (CloudCodeException ex)
			{
				HandleCloudCodeException(ex);
