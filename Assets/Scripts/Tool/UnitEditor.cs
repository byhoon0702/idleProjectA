...
public class UnitEditor : MonoBehaviour
{
	...
	public void OnClickSpawn(UnitData data)
	{
		...
		if (isDummyShow)
		{
			unit.transform.localPosition = new Vector3(-2.5f, 0, 0);
		}
		else
		{
	...
	public List<string> GetStates()
	{
		...
		//for(int i=0; i< editorAnimatorController.layers.Length; i++)
		//{
		//	//var param = editorAnimatorController.parameters[i];
		//}

		//viewPlayer.PlayAnimation(
		for (int i = 0; i < editorAnimatorController.layers.Length; i++)
		{
			var layer = editorAnimatorController.layers[i];
	...
	public void SpawnTargetDummy(int count)
	{
		...
		{
			GameObject unit = new GameObject("Dummy");
			unit.transform.SetParent(root2d);
			unit.transform.localPosition = new Vector3(2.5f + (1.5f * i), 0, 0);
			DummyUnit dummy = unit.AddComponent<DummyUnit>();
	...
	private void Update()
	{
		...
		if (Input.GetMouseButtonDown(0))
		{
			...
			for (int i = 0; i < dummies.Count; i++)
			{
				if (Vector3.Distance(ray.origin, dummies[i].unitAnimation.CenterPivot.position) < 1f)
				{
