...
namespace UnityEngine.UI.Extensions
{
	...
	public class ScrollPositionController : UIBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
	{
		#region Sub-Classes
		[System.Serializable]
		public class UpdatePositionEvent : UnityEvent<float, bool>
		{
		}
		...
		Action<int, float, bool> OnCompleteScroll;
		int wait_scroll_index;
		float wait_scroll_duration;
		...
		void IBeginDragHandler.OnBeginDrag(PointerEventData eventData)
		{
			...
			Camera event_camera = viewport_camera;
			if (event_camera == null)
			{
				event_camera = eventData.pressEventCamera;
			}
		...
		void UpdatePosition(float position, bool forceUpdate)
		{
			currentScrollPosition = position;

			if (OnUpdatePosition != null)
			{
				OnUpdatePosition.Invoke(currentScrollPosition, forceUpdate);
			}
			m_forceUpdate = false;
		}
		...
		{
			this.currentScrollPosition = startIndex;
		}
		...
		void Update()
		{
			...
			if (autoScrolling)
			{
				...
				UpdatePosition(position, m_forceUpdate);

				if (Mathf.Approximately(alpha, 1f))
				{
					autoScrolling = false;
					if (OnCompleteScroll != null)
					{
						OnCompleteScroll.Invoke(wait_scroll_index, wait_scroll_duration, m_forceUpdate);
						wait_scroll_index = 0;
						wait_scroll_duration = 0;

						OnCompleteScroll = null;
					}
		...
		public void ScrollTo(int index, float duration, bool force_update = false)
		{
			velocity = 0;
			autoScrolling = true;
			...
			dragStartScrollPosition = currentScrollPosition;
			m_forceUpdate = force_update;
			autoScrollPosition = movementType == MovementType.Unrestricted
				? CalculateClosestPosition(index)
				: index;
		}

		public void ScrollTo(int index, int jump_index, float duration, bool force_update = false)
		{
			// 멀리서 스크롤할 때 in_remain_count 만큼 순간이동 한 뒤 in_index까지 스크롤 하는 기능
			wait_scroll_index = index;
			wait_scroll_duration = duration;

			OnCompleteScroll = ScrollTo;
			...
			m_forceUpdate = force_update;
			autoScrollPosition = movementType == MovementType.Unrestricted
				? CalculateClosestPosition(jump_index)
				: jump_index;
		}
