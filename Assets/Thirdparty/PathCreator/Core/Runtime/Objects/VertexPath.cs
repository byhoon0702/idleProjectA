...
namespace PathCreation
{
	...
	public class VertexPath
	{
		...
		public TimeOnPathData GetTimeOnPathData(float dst, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Loop)
		{
			float t = dst / length;
			var data = CalculatePercentOnPathData(t, endOfPathInstruction);
			return data;
		}
		public bool GetPointAtTimeUntilIndex(Transform in_transform, float dst, int index, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Loop)
		{
			float t = dst / length;
			var data = CalculatePercentOnPathData(t, endOfPathInstruction);
			if (data.nextIndex > index)
			{
				in_transform.position = Vector3.Lerp(GetPoint(data.previousIndex), GetPoint(index), data.percentBetweenIndices);
				return false;
			}
			in_transform.position = Vector3.Lerp(GetPoint(data.previousIndex), GetPoint(data.nextIndex), data.percentBetweenIndices);
			return true;
		}
		...
		public Vector3 GetClosestLocalPointOnPath(Vector3 localPoint)
		{
			// Transform the provided worldPoint into VertexPath local-space.
			// This allows to do math on the localPoint's, thus avoiding the need to
			// transform each local vertexpath point into world space via GetPoint.
		...
		public int GetClosestIndex(Vector3 localPoint)
		{
			TimeOnPathData data = CalculateClosestPointOnPathData(localPoint);
			return data.nextIndex;
		}

		public Vector3 GetLocalPositionOnIndex(int index)
		{
			if (index == -1 || localPoints.Length - 1 < index)
			{
				return Vector3.zero;
			}
		...
		public float GetClosestTimeOnLocalPath(Vector3 localPoint)
		{
			TimeOnPathData data = CalculateClosestPointOnPathData(localPoint);
			return Mathf.Lerp(times[data.previousIndex], times[data.nextIndex], data.percentBetweenIndices);
		}
		...
		public float GetClosestDistanceAtWorldPointAlongPath(Vector3 worldPoint)
		{
			TimeOnPathData data = CalculateClosestPointOnPathData(worldPoint);
			return Mathf.Lerp(cumulativeLengthAtEachVertex[data.previousIndex], cumulativeLengthAtEachVertex[data.nextIndex], data.percentBetweenIndices);
		}

		public float GetClosestDistanceAlongLocalPath(Vector3 localPoint)
		{
