...
public class LeanAudio : object
{
	...
	private log4net.ILog m_logger = gplat.Log.logger("lean_audio");
    ...
    */
	public static AudioClip createAudio(AnimationCurve volume, AnimationCurve frequency, LeanAudioOptions options = null)
	{
		...
		int generatedWavePtsLength = createAudioWave(volume, frequency, options);
		// m_logger.Info("generatedWavePtsLength:"+generatedWavePtsLength);
	...
	private static int createAudioWave(AnimationCurve volume, AnimationCurve frequency, LeanAudioOptions options)
	{
		...
		for (int i = 0 ; i < PROCESSING_ITERATIONS_MAX ; i++)
		{
			...
			// m_logger.Info("i:"+i+" f:"+f+" passed:"+passed+" height:"+height+" time:"+time);
	...
	private static AudioClip createAudioFromWave(int waveLength, LeanAudioOptions options)
	{
		...
		for (int i = 0 ; i < audioArr.Length ; i++)
		{
			...
			if (options.modulation != null)
			{
				for (int k = 0 ; k < options.modulation.Length ; k++)
				{
					...
					// if(k<10){
					// m_logger.Info("k:"+k+" peakMulti:"+peakMulti+" value:"+value+" after:"+(value*peakMulti));
					// }
					value *= peakMulti;
				}
			}
			...
			audioArr[i] = value;
			// m_logger.Info("pt:"+pt+" i:"+i+" val:"+audioArr[i]+" len:"+audioArr.Length);
		}
		...
		else
		{
			...
			// m_logger.Info("len:"+audioArr.Length+" lengthSamples:"+lengthSamples+" freqRate:"+options.frequencyRate);
	...
	public static AudioClip generateAudioFromCurve(AnimationCurve curve, int frequencyRate = 44100)
	{
		...
		// m_logger.Info("curveTime:"+curveTime+" AudioSettings.outputSampleRate:"+AudioSettings.outputSampleRate);
		for (int i = 0 ; i < audioArr.Length ; i++)
		{
	...
	{
		// m_logger.Info("audio length:"+audio.length);
	...
	{
		// m_logger.Info("Audio channels:"+audioClip.channels+" frequency:"+audioClip.frequency+" length:"+audioClip.length+" samples:"+audioClip.samples);
