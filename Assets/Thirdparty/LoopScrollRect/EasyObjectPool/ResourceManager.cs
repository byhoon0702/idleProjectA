...
namespace SG
{
	...
	public class ResourceManager : MonoBehaviour
	{
		...
		public void InitPool(GameObject in_prefab, int size, PoolInflationType type = PoolInflationType.DOUBLE)
		{
			if (poolDict.ContainsKey(in_prefab.name))
			{
				return;
			}
			else
			{
				GameObject pb = Instantiate(in_prefab);
				if (pb == null)
				{
					Debug.LogError("[ResourceManager] Invalide prefab name for pooling :" + in_prefab.name);
					return;
				}
				poolDict[in_prefab.name] = new Pool(in_prefab.name, pb, gameObject, size, type);
				Destroy(pb);
			}
		}
